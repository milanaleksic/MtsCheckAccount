 ...
 *
 *  Ported to Groovy by milan dot aleksic at gmail dot com
 *
 **************************************************************************/

public class PDUConverter {

    private static char NPC7 = (char) 63
    private static char NPC8 = (char) '?'
    ...
    private static int[] lookup_ascii8to7 = [
            NPC7,       /*     0      null [NUL]                              */
     ...
     *************************************************************************** */
    private static int[] lookup_ascii7to8 = [
            64,         /*  0      @  COMMERCIAL AT                           */
            163,        /*  1      £  POUND SIGN                              */
            36,         /*  2      $  DOLLAR SIGN                             */
            165,        /*  3      ¥  YEN SIGN                                */
    ...
    private static String convert7BitToAscii(String a7bit) {
        assert a7bit
        final int STATE_NORMAL = 0, STATE_ESCAPED = 2
        int state = STATE_NORMAL
        char converted
        StringBuilder builder = new StringBuilder()
        a7bit.bytes.each { byte ch ->
            switch (state) {
                case STATE_NORMAL:
                    converted = (char) lookup_ascii7to8[(int) ch]
                    if (converted != 27) {
                        builder << (char) converted
                    } else {
                        state = STATE_ESCAPED
                    }
                    break
    ...
    private static String convertAsciiTo7Bit(String ascii) {
        assert ascii
        StringBuilder builder = new StringBuilder()
        ascii.each { ch ->
            def converted = lookup_ascii8to7[(int) ch]
            if (converted < 256) {
                builder << (char) Math.abs(converted)
            } else {
                builder << (char) 27
                builder << (char) (converted - 256);
            }
        }
        return builder.toString()
    }

    public static convertPDUToAscii(String pdu) {
        assert pdu
        // Allocate sufficient memory for the 7-bit string
        StringBuilder ascii7bit = new StringBuilder()
        int prevB = 0
        for (int r=0; r<pdu.size()/2; r++) {
            int b = Integer.parseInt(pdu[r*2..r*2+1], 16)
            if (r % 7 == 0) {
                ascii7bit << ((char) (b << 0) & 0x7F)
            } else if (r % 7 == 6) {
                ascii7bit << ((char) ((b << 6) | (prevB >> 2)) & 0x7F)
                ascii7bit << ((char) ((b >> 1) & 0x7F))
            } else {
                ascii7bit << ((char) ((b << (r % 7)) | (prevB >> (7 + 1 - (r % 7)))) & 0x7F)
            }
            prevB = b
        }
        return convert7BitToAscii(ascii7bit.toString())
    }

    private static final String HEXES = "0123456789ABCDEF";

    public static String getHex(byte b) {
        return HEXES.charAt((b & 0xF0) >> 4).toString() + HEXES.charAt((b & 0x0F)).toString();
    }

    public static String convertAsciiToPDU(String ascii) {
        assert ascii
        // Start by converting the ISO-string to a 7bit-string
        String ascii7bit = convertAsciiTo7Bit(ascii)
        StringBuilder builder = new StringBuilder()
        int r = 0
        int w = 0
        while (r < ascii7bit.size()) {
            if (r < ascii7bit.size() - 1)
                builder << getHex((byte)((ascii7bit.bytes[r] >> (w % 7)) & 0x7F) | ((ascii7bit.bytes[r + 1] << (7 - (w % 7))) & 0xFF))
            else
                builder << getHex((byte)((ascii7bit.bytes[r] >> (w % 7)) & 0x7F))
            if ((w % 7) == 6)
                r++
            r++
            w++
    ...
    public static void main(String[] args) {
        String test = '*123456789#ABCDEFGHIJKLMNOPQRSTUVWXYZ{~|\\'
